 ...
 *
 * Mobile  (<1024px): label → title → buttons apilados en columna (full-width).
 * Desktop (≥1024px): label + title a la izquierda, botones inline a la derecha.
 */
export default function InventoryHeader({ onAddProduct, onImport }) {
  return (
    <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between lg:gap-6">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:gap-2 shrink-0">
        {/* Primary action – Add product */}
        <button
          id="add-product-btn"
          onClick={onAddProduct}
          className="flex items-center justify-center gap-2 w-full lg:w-auto
                     py-3 px-5 bg-green-900 hover:bg-green-800 active:bg-green-950
        ...
        <button
          ...
          onClick={onImport}
          className="flex items-center justify-center gap-2 w-full lg:w-auto
                     py-3 px-5 bg-white hover:bg-stone-50 active:bg-stone-100
